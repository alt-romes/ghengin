...
main = do
  ...
  withLinearIO $
   runCore (width, height) Linear.do
     ...
     (gridMeshX, pipeline) <- (createMesh pipeline grid_props (toSV $ gridVertsX width height) ↑)
     (gridMeshY, pipeline) <- (createMesh pipeline grid_props (toSV $ gridVertsY width height) ↑)
     (axisMesh, pipeline) <- (createMesh pipeline axis_props (toSV $ axisVerts width height) ↑)
     (functionMesh, pipeline) <- (createMesh pipeline line_props (toSV verts) ↑)
     ...
     (functionMesh, pipeline2) <- (createMesh pipeline2 line_props (toSV verts) ↑)
