...
pointOnPlanet PlanetShape{..} pointOnUnitSphere =
  let elevation = evalNoise (unCollapsible planetNoise) pointOnUnitSphere
      finalElevation = inRangeVal planetRadius * (1 + (max 0 elevation))
...
biomeOnPoint PlanetColor{..} pointOnUnitSphere =
  let noise = evalNoise (unCollapsible biomesNoise) pointOnUnitSphere
   ...
   in biomeVal / (max 1 (fromIntegral (length planetBiomes - 1)))
...
newPlanetMesh :: _ -- more constraints
              ...
              => RenderPipeline π bs
               ⊸ Planet
              -> Renderer ((PlanetMesh, RenderPipeline π bs), Linear.Ur MinMax)
newPlanetMesh rp Planet{..} = Linear.do
  ...
  let UnitSphere us is0 = newUnitCubeSphere (inRangeVal planetShape.planetResolution)
   ...
   in (, Linear.Ur minmax) Linear.<$> createMeshWithIxsSV rp (DynamicBinding (Linear.Ur mempty) :## GHNil) (V.convert planetVs) (SV.map fromIntegral is)
...
newPlanetMaterial mm pl planet = Linear.do
  ...
  material @_ @π (StaticBinding (Linear.Ur mm) :## Texture2DBinding tex :## GHNil) pl
...
planetTexture PlanetColor{planetBiomes, planetColorsInterpolate} = Linear.do
  ...
  let gradientImg = generateImage pixelPaint
                      ...
                      (length planetBiomes){-height=N pixels for a N biomes-}
      pixelPaint x y =
        let Collapsible biome = planetBiomes !! y
  ...
  -- Linear.liftSystemIO $ savePngImage "my_generated_gradient_texture.png" (ImageRGBA8 gradientImg)
  ...
  if interpolate then
    case find (\(InRange k, _) -> value <= k) colors of
      ...
      Just (InRange k2, Color c2) ->
        let (InRange k1, Color c1) = case reverse $ takeWhile (\(InRange k, _) -> value > k) colors of
              [] -> case colors of
