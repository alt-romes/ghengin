...
newPlanet (PlanetSettings re ra co bo nss) = do
  ...
  ps' <- forM vs $ \(Vertex p _ _) -> do
    case nss of
      NoiseSettings nl st ro bro pers ce mv en NE.:| nss' -> do
        initialElevation <- evaluateNoise <$> get nl <*> get st <*> get ro <*> get bro <*> get pers <*> get ce <*> get mv <*> get en <*> pure p
